...
Fit a kNN model and use a 10 fold cross-validation. Train the kNN Model by repeating the 10 fold cross-validation 3 times.
...
```{r}
stat1 <- round(confusionMatrix(data = heartTest2$HeartDisease, reference = predict(treeFit1, newdata = heartTest2))$overall,4)
stat2 <- round(confusionMatrix(data = heartTest2$HeartDisease, reference = predict(treeFit2, newdata = heartTest2))$overall,4)
stat3 <- round(confusionMatrix(data = heartTest2$HeartDisease, reference = predict(treeFit3, newdata = heartTest2))$overall,4)

#Combine the statistics to view in a table
fitTreeStats <- data.frame(rpart = stat1, rf = stat2, boosted = stat3)
fitTreeStats
```
